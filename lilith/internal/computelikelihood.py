...
from effectivemu import compute_effective_mu
from priors import evaluate_prior

def compute_likelihood(exp_mu, user_mu, alpha):
    ...
    for mu in exp_mu:
        ...
        effective_mu = user_mu
        if mu["type"] == "eff":
            effective_mu = compute_effective_mu(user_mu, mu, alpha)
        ...
        try:
            ...
            for (prod,decay),eff_prod in mu["eff"]["x"].items():
                user_mu_effscaled["x"] += eff_prod*effective_mu[prod,decay]

            if mu["dim"] == 2:
                user_mu_effscaled["y"] = 0.
                for (prod,decay),eff_prod in mu["eff"]["y"].items():
                    user_mu_effscaled["y"] += eff_prod*effective_mu[prod,decay]
            ...
            if mu["type"] == "f" or mu["type"] == "eff":
                if mu["dim"] == 1:
                    ...
                    cur_l = (mu["Lxy"](user_mu_effscaled["x"],
                            ...
                            - mu["LChi2min"])
                    if mu["type"] == "eff":
                        l_prior = evaluate_prior(mu,alpha)
                        cur_l += l_prior                              

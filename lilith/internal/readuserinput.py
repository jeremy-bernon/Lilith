...
class ReadUserInput:
    ...
    def __init__(self, inputstring):
        ...
        self.mu = []
        # self.alpha is a list of systematical uncertainties
        self.alpha={}
    ...
    def get_nextreducedcouplings(self):
        ...
        accepted_tag = ["val","mean","sigma"]
        accepted_prior = ["normal","lognormal"]
        ...
        for child in red_coupl:
            ...
            if child.tag == "alpha":
                if "syst" not in child.attrib:
                    raise UserInputError('<alpha> tag has no attribute syst')
                else:
                    alpha_cur = {}
                    if "prior" in child.attrib:
                        if child.attrib["syst"] in self.alpha:
                            self.warning('alpha syst ="' + child.attrib["syst"] +
                                        '" is being redefined')
                        if child.attrib["prior"] in accepted_prior:
                            try:
                                alpha_cur["prior"] = child.attrib["prior"]
                                for subchild in child:
                                    if subchild.tag in accepted_tag:
                                        alpha_cur[subchild.tag]=float(subchild.text)
                                    else:
                                        raise UserInputError('<'+subchild.tag+'> not allowed in '+
                                                             '<alpha> tag (val, mean, sigma only)')
                                self.alpha[child.attrib["syst"]] = alpha_cur
                            except TypeError: # empty tag is of type NULL
                                self.warning('<alpha> tag to "' + child.attrib["syst"] +
                                         '" is missing <val>, <mean> or <sigma> value; setting it to 0 with flat prior')
                                self.alpha[child.attrib["syst"]] = {"val":0.}
                            except ValueError:
                                raise UserInputError(
                                    'value of the <alpha> tag for "' +
                                    child.attrib["syst"] + '" is not a ' +
                                    'number.')
                    else:
                        if child.attrib["syst"] in self.alpha:
                            self.warning('alpha syst ="' + child.attrib["syst"] +
                                        '" is being redefined')
                        try:
                            self.alpha[child.attrib["syst"]] = {"val":float(child.text)}
                        except TypeError: # empty tag is of type NULL
                            self.warning('<alpha> tag to "' + child.attrib["syst"] +
                                         '" is empty; setting it to 0')
                            self.alpha[child.attrib["syst"]] = {"val":0.}
                        except ValueError:
                            raise UserInputError(
                                'value of the <alpha> tag for "' +

...
class ReadUserInput:
    ...
    def __init__(self, inputstring):
        ...
        self.mu = []
        # self.mutemplate is for mutemplate mode only
        # it is a list of the phi and eta coefficients
        # for the various systematic uncertainties
        self.mutemplate = []

        for i in xrange(n_syst):
            ...
            elif self.mode == "mutemplate":
                self.mutemplate.append(self.get_nextsystematicuncertainties())

        print self.mutemplate
    ...
    def getmode(self):
        ...
        for child in self.root:
            ...
            if child.tag not in ReadUserInput.modes:
                ...
                if curmode == "":
                    ...
                    if "syst" in child.attrib and child.attrib["syst"] != "":
                       name_syst.append(child.attrib["syst"])
                else:
        ...
        if curmode == "":
            raise UserInputError(
                ...
                '<signalstrengths>, <reducedcouplings> or <mutemplate>)')

        # check if two higgses have the same name
        if any(name_syst.count(x) > 1 for x in name_syst):
            self.warning('there are several systematic uncertainties with the same name; ' +
                         'we will consider that all systematic are different ' +
                         'and contribute to the likelihood')
 
        return curmode, num_syst
    ...
    def get_nextsystematicuncertainties(self):
        """..."""
        ...
        mutemplate_block = None

        n = 0
        for child in self.root:
            if child.tag != "mutemplate":
                continue
            if n == len(self.mutemplate):
                mutemplate_block = child
                if "syst" in child.attrib and child.attrib["syst"] != "":
                    mutemplatep["extra"]["name"] = child.attrib["syst"]
            n += 1
        ...
        for child in mutemplate_block:
            if child.tag == "eta":
                if "p" not in child.attrib or "pprime" not in child.attrib:
                    ...
                    if (p,pprime) in mutemplatep:
                        self.warning('<eta> tag with p="' + p +
                                     '" and pprime="' + pprime + '" is being ' +
                                     'redefined')

                    try:
                        mutemplatep[p,pprime] = float(child.text)
                    except TypeError: # empty tag is of type NULL
                        mutemplatep[p, pprime] = 0.
                        self.warning('<eta> tag with p="' +
                                     p + '" and pprime="' +
                                     pprime + '" is empty; ' +
                                     'setting it to 0')
                    ...
                    if ("phi") in mutemplatep:
                        self.warning('<phi> tag is being ' +
                                     'redefined')

                    try:
                        mutemplatep[("phi")] = float(child.text)
                    except TypeError: # empty tag is of type NULL
                        mutemplatep[("phi")] = 0.
                        ...
                        raise UserInputError(
                            'value of the <phi> tag is not a number.')
        ...
        # -- first: p
        for multip, p_list in multiprod.items():
            self.check_multiprod(mutemplatep, multip, p_list)

        for multip, p_list in multiprod2.items():
            self.check_multiprod(mutemplatep, multip, p_list)

        # now, one can delete all multiprod labels
        for key,mu_value in mutemplatep.items():
            if key == "extra" or key == "phi":
                continue
            p,pprime = key
            if p in multiprod or p in multiprod2:
                del mutemplatep[p,pprime]

        # -- then: pprime
        for multip, p_list in multiprod.items():
            self.check_multiprodprime(mutemplatep, multip, p_list)
        ...
        for key,mu_value in mutemplatep.items():
            if key == "extra" or key == "phi":
                continue
            p,pprime = key
            if pprime in multiprod or pprime in multiprod2:
                del mutemplatep[p,pprime]
        ...
        for (p,pprime) in mandatory_mutemplate:
            if (p,pprime) not in mutemplatep:
                mutemplatep[p,pprime] = 0.
#                if prod in warning_prod and decay in warning_decay:
#                    self.warning('signal strength for prod "' + prod +
#                             '" and decay "' + decay + '" is not  specified; ' +
#                             'fixing it to SM value=1')
        if ("phi") not in mutemplatep:
          mutemplatep[("phi")]=0.
    ...
    def check_multiprodprime(self, mup, multip, p_list):
        """for signal strengths mode"""
        ...
        for key,mu_value in mup.items():
            ...
            p,pprime = key
            if pprime == multip:
                if set([(p,subprod) for subprod in p_list]).issubset(mup):
                    # if multiprod and individual prod are all
                    # defined, check consistency
                    for subprod in p_list:
                        if mu_value != mup[p,subprod]:
                            self.warning('inconsistent definition of the ' +
                                         'eta p="' + multip + '" and "' +
                                         pprime + '" when p="' + pprime + '"; ' +
                                         'skipping ' + 'eta pprime="' + multip + '" tag')
                else:
                    # only multiprod label is present, or multiprod
                    # label and tags for only part of the associated prod
                    ...
                    for (p,subprod) in given_mu:
                        if mu_value != mup[p,subprod]:
                            raise UserInputError(
                                'inconsistent definition of eta for pprime "' +
                                pprime + '" when p is "' + p + '"')
            
                    if len(given_mu) == 0:
                        # only multiparticle tag is defined
                        for subprod in p_list:
                            mup[p,subprod] = mup[p,multip]
                    else:
                        # when multiprod tag and part of the prod tags
                        # are given, with equal values
                        other_mu = [(p,subprod) for subprod in p_list if
                                    (p,subprod) not in mup]
                        for (p,op) in other_mu:
                            mup[p,op] = mup[p,multip]
                        self.warning('signal strengths with pprime ' + pprime +
                                     ' and ' +
                                     str(given_mu) + ' are all defined and ' +
                                     'equal; assuming that the signal strengths to '+
                                     str(other_mu) + ' are the same [for p = "' +
                                     p + '"]')
